...
I have refactored `app.py` to implement persistent signal history, fix a critical `ValueError`, improve chart visualization, and add a safeguard against race conditions.

## Changes

### 1. JSON Persistence
- Implemented `load_history` and `save_history` to manage `trade_history.json`.
...
- **`get_data`**: Added logic to flatten MultiIndex columns from `yfinance` to ensure a simple DataFrame structure.
- **`analyze_london_breakout`**: Forced `float()` conversion for `current_price`, `ema_50`, `box_high`, and `box_low` to prevent "ambiguous truth value" errors.
